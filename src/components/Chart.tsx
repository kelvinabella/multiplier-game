...
export default function Chart() {
	const { speedController, gameStarted, multiplierValue, setGameEnded, setGameStarted } = useGameContext();
	...
	const { value, reset } = useCountUp({
		...
		duration: 100 - speedController.current + 2,
	});

	useEffect(() => {
		if (gameStarted) {
			animate(scope.current, { opacity: 0 }, { duration: 0.3, ease: "easeInOut" });
			animate(scope.current, { pathLength: 0 }, { duration: 0.3, ease: "easeInOut" });
		}
	...
	useEffect(() => {
		if (value) {
			...
			animate(scope.current, { opacity: 1 }, { duration: 1, ease: "easeInOut" });
